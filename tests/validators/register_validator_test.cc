...
TEST(validators_register_validator_test, on_sessions_port_empty) {
...
TEST(validators_register_validator_test, on_wrong_sessions_port_primitive) {
    const auto _state = std::make_shared<state>();
...
TEST(validators_register_validator_test, on_clients_port_empty) {
    const auto _state = std::make_shared<state>();
...
TEST(validators_register_validator_test, on_wrong_clients_port_primitive) {
    const auto _state = std::make_shared<state>();
...
TEST(validators_register_validator_test, on_registered_empty) {
    const auto _state = std::make_shared<state>();
    ...
    ASSERT_TRUE(_response->get_data().at("data").as_object().at("params").is_string());
    ASSERT_EQ(_response->get_data().at("data").as_object().at("params").as_string(),
              "params registered attribute must be present");
}

TEST(validators_register_validator_test, on_wrong_registered_primitive) {
    const auto _state = std::make_shared<state>();
    ...
    ASSERT_TRUE(_response->get_data().at("data").as_object().at("params").is_string());
    ASSERT_EQ(_response->get_data().at("data").as_object().at("params").as_string(),
              "params registered attribute must be boolean");
}
